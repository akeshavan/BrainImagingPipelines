...
"""
desc = """
Group Analysis: Multiple Regression UCSF
========================================

"""
mwf = MetaWorkflow()
mwf.uuid = '852c6984830c11e3880000212814c10e'
...
def create_config():
    ...
    c.datagrabber = Data(['copes','varcopes','mask'])
    ...
    c.datagrabber.field_template = dict(copes='%s/preproc/output/fwhm_%s/cope*.nii.gz',
        varcopes='%s/preproc/output/fwhm_%s/varcope*.nii.gz',mask='%s/mask.nii')
    c.datagrabber.template_args = dict(copes=[['fwhm',"contrast","subject_id"]],
        varcopes=[['fwhm',"contrast","subject_id"]],mask=[["subject_id"]])
...
def create_2lvl(name="group",mask=None):
    ...
    inputspec = pe.Node(niu.IdentityInterface(fields=['copes','varcopes','group',
                                                      'template', "contrasts",
                                                      "regressors","run_mode","mask"]),name='inputspec')
    ...
    if mask ==None:
        ...
        wk.connect(inputspec,'mask',flame,'mask_file')
    ...
    if mask ==None:
        ...
        wk.connect(inputspec,'mask',outputspec,'mask')
...
def create_2lvl_rand(name="group_randomize",mask=None,iters=5000):
    ...
    inputspec = pe.Node(niu.IdentityInterface(fields=['copes','varcopes','run_mode',
                                                      'template', "contrasts","group",
                                                      "regressors","mask"]),name='inputspec')
    ...
    if mask == None:
        ...
        wk.connect(inputspec,'mask',rand,'mask')
...
def connect_to_config(c):
    ...
    if c.run_mode == 'ols' and not c.do_randomize:
    ...
    if c.use_mask:
        wk.connect(datagrabber,"datagrabber.mask",inputspec,"mask")
