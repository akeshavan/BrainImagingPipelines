...
desc = """
7T fMRI multiband Resting Preprocessing
===================-===================
...
"""
mwf = MetaWorkflow()
mwf.uuid = 'a7724b169a7311e3a30a00144fd208cc'
mwf.tags = ['fMRI','preprocessing','resting','7T','multiband']
mwf.help = desc
...
def simple_preproc(c):
    ...
    strip_rois = pe.MapNode(fsl.ExtractROI(),name="remove_10_timpoints",iterfield=["in_file"])
    strip_rois.inputs.t_size = -1
    strip_rois.inputs.t_min = 10


    motion_correct = pe.Node(nipy.FmriRealign4d(),name="realign")
    motion_correct.inputs.tr = c.TR
    motion_correct.inputs.slice_order = c.SliceOrder
    motion_correct.inputs.time_interp = True
    motion_correct.inputs.loops = c.loops
    motion_correct.inputs.speedup = c.speedup
    ...
    wf.connect(img2float,'out_file', strip_rois,'in_file')
    wf.connect(strip_rois,"roi_file",motion_correct,"in_file")
    ...
    meanfunc.inputs.inputspec.parameter_source = "NiPy"
